...
const TagContainer = styled.div`
  ...
  background: linear-gradient(
    to bottom,
    ${({ theme }) => theme.palette.alternate.base} 0 4px,
    ${({ theme }) => theme.palette.main.saturated} 4px
  );
  height: 70px;
  position: relative;
  margin: 1rem 2rem;
  ...
  box-shadow: 14px 5px 8px 0px rgba(2, 6, 80, 0.3);
...
const Tag = ({ title, count }) => {
  ...
  return (
    <TagContainer>
      <Link to={`/categories/${linkLocation}`}>
        <span />
        <div>{title}</div>
        <small>{count}</small>
        <span />
